...
//    splice_info_section->splice_command_printer(splice_info_section->splice_command_ptr);
        ...
        for (int i = 0; i < splice_info_section->segmentation_descriptor_count; i++) {
            ...
            indent_print(LVL_3, "splice_descriptor_tag : %d",
                         splice_info_section->segmentation_descriptor[i].descriptor.splice_descriptor_tag);
            indent_print(LVL_3, "descriptor_length : %d",
                         splice_info_section->segmentation_descriptor[i].descriptor.descriptor_length);
            indent_print(LVL_3, "identifier : %d",
                         splice_info_section->segmentation_descriptor[i].descriptor.identifier);
            indent_print(LVL_3, "segmentation_event_id : %d",
                         splice_info_section->segmentation_descriptor[i].segmentation_event_id);
            indent_print(LVL_3, "segmentation_event_cancel_indicator : %d",
                         splice_info_section->segmentation_descriptor[i].segmentation_event_cancel_indicator);
            ...
            if (splice_info_section->segmentation_descriptor[i].segmentation_event_cancel_indicator == 0) {
                indent_print(LVL_3, "program_segmentation_flag : %d",
                             splice_info_section->segmentation_descriptor[i].program_segmentation_flag);
                indent_print(LVL_3, "segmentation_duration_flag : %d",
                             splice_info_section->segmentation_descriptor[i].segmentation_duration_flag);
                indent_print(LVL_3, "delivery_not_restricted_flag : %d",
                             splice_info_section->segmentation_descriptor[i].delivery_not_restricted_flag);
                if (splice_info_section->segmentation_descriptor[i].delivery_not_restricted_flag == 0) {
                    indent_print(LVL_3, "web_delivery_allowed_flag : %d",
                                 splice_info_section->segmentation_descriptor[i].web_delivery_allowed_flag);
                    indent_print(LVL_3, "no_regional_blackout_flag : %d",
                                 splice_info_section->segmentation_descriptor[i].no_regional_blackout_flag);
                    indent_print(LVL_3, "archive_allowed_flag : %d",
                                 splice_info_section->segmentation_descriptor[i].archive_allowed_flag);
                    indent_print(LVL_3, "device_restrictions : %d",
                                 splice_info_section->segmentation_descriptor[i].device_restrictions);
                } else {
                    indent_print(LVL_3, "reserved_flags : %d",
                                 splice_info_section->segmentation_descriptor[i].reserved_flags);
                }
                if (splice_info_section->segmentation_descriptor[i].program_segmentation_flag == 0) {
                    indent_print(LVL_3, "component_count : %d",
                                 splice_info_section->segmentation_descriptor[i].component_count);
                    for (int j = 0; j < splice_info_section->segmentation_descriptor[i].component_count; j++) {
                        indent_print(LVL_3, "{");
                        indent_print(LVL_4, "component_tag : %d",
                                     splice_info_section->segmentation_descriptor[i].component_tags[j].component_tag);
                        indent_print(LVL_4, "reserved : %d",
                                     splice_info_section->segmentation_descriptor[i].component_tags[j].reserved);
                        indent_print(LVL_4, "pts_ofset : %llu",
                                     splice_info_section->segmentation_descriptor[i].component_tags[j].pts_ofset);
                        indent_print(LVL_3, "}");
                    }
                } // program_segmentation_flag == 0

                if (splice_info_section->segmentation_descriptor[i].segmentation_duration_flag == 1) {
                    indent_print(LVL_3, "segmentation_duration : %llu",
                                 splice_info_section->segmentation_descriptor[i].segmentation_duration);
                } // segmentation_duration_flag == 1
                indent_print(LVL_3, "segmentation_upid_type : %d",
                             splice_info_section->segmentation_descriptor[i].segmentation_upid_type);
                indent_print(LVL_3, "segmentation_upid_length : %d",
                             splice_info_section->segmentation_descriptor[i].segmentation_upid_length);
                ...
                for (int k = 0; k < splice_info_section->segmentation_descriptor[i].segmentation_upid_length; k++) {
                    indent_print(LVL_3, "segmentation_upid[%d] : 0x%02X", k,
                                 splice_info_section->segmentation_descriptor[i].segmentation_upid_data[k]);
                }
                indent_print(LVL_3, "segmentation_type_id : %d",
                             splice_info_section->segmentation_descriptor[i].segmentation_type_id);
                indent_print(LVL_3, "segment_num : %d", splice_info_section->segmentation_descriptor[i].segment_num);
                indent_print(LVL_3, "segments_expected : %d",
                             splice_info_section->segmentation_descriptor[i].segments_expected);
                if (splice_info_section->segmentation_descriptor[i].segmentation_type_id == 0x34 ||
                    ...
                    indent_print(LVL_3, "sub_segment_num : %d",
                                 splice_info_section->segmentation_descriptor[i].sub_segment_num);
                    indent_print(LVL_3, "sub_segments_expected : %d",
                                 splice_info_section->segmentation_descriptor[i].sub_segments_expected);
                }
